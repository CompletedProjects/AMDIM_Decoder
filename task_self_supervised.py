...
def _train(model, optim_inf, scheduler_inf, checkpointer, epochs,
    ...
    for epoch in range(next_epoch, epochs):
        ...
        for _, ((images1, images2), labels) in enumerate(train_loader): # Nawid - obtains the images and the labels
            ...
            loss_cls = (loss_xent(lgt_glb_mlp, labels) + # Nawid - Loss for the classifier terms
            ...
            loss_opt = loss_inf + loss_cls # Nawid - Total loss is the loss from the global to local prediction as well as the loss from the classifier predictions
            optim_inf.zero_grad()
