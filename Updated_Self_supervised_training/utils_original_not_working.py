...
from task_decoder import save_reconstructions
...
def test_decoder_model(model, test_loader, device, stats, max_evals=200000):
    ...
    for _, (images, labels) in enumerate(test_loader): # Nawid - Iterates through test loader images and labels
        ...
        labels = labels.cpu()
        with torch.no_grad():
            res_dict = model(x1=images, x2=images, decoder_only=True) # Nawid - Input images into the model
    ...
    save_reconstructions(images,image_reconstructions, training = False) # Nawid - Used to save the training images
