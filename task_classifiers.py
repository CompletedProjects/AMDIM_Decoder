...
def _train(model, optimizer, scheduler, checkpointer, epochs, train_loader,
    ...
    total_updates = 0
    next_epoch, total_updates = checkpointer.get_current_position(classifier=True) # Nawid - I think if I am continuing training, then it finds the current epoch it is on
    for epoch in range(next_epoch, epochs):
        ...
        for _, ((images1, images2), labels) in enumerate(train_loader): # Nawid - loads the two different images
            ...
            lgt_glb_mlp, lgt_glb_lin = res_dict['class'] # Nawid-  Obtains the logits from the mlp and the linear
            # compute total loss for optimization
            loss = (loss_xent(lgt_glb_mlp, labels) + # Nawid-  Compute the loss using the mlp and the linear layer - there is no loss term related to the encoder and the input to the loss term is the logits which
            ...
            update_train_accuracies(epoch_stats, labels, lgt_glb_mlp, lgt_glb_lin) # Nawid -  updates the accuracies
...
def train_classifiers(model, learning_rate, dataset, train_loader,
    ...
    for mod in model.class_modules: # Nawid - This only looks at the classifier modules
        # reset params in the evaluation classifiers
        mod.apply(weight_init) # Nawid -Reset parametes
    mods_to_opt = [m for m in model.class_modules] # Nawid - Makes a list of the modules to perform the optimisation on which in this case is just the classifier modules
    # configure optimizer
